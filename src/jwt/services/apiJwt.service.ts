...
export class ApiJwtService implements JWTService {
  ...
  public tokenExpirationDate(tokenType: TokenType): Date {
    switch (tokenType) {
      case TokenType.access:
        const accessTokenExpirationDate: Optional<Date> = parseDayString(this.config.accessTokenExpiresIn);
        if (!accessTokenExpirationDate) {
          throw new Error(`${this.config.accessTokenExpiresIn} is not a valid access tokeb date string.`);
        }
        return accessTokenExpirationDate;
      case TokenType.refresh:
        const refreshTokenExpirationDate: Optional<Date> = parseDayString(this.config.refreshTokenExpiresIn);
        if (!refreshTokenExpirationDate) {
          throw new Error(`${this.config.refreshTokenExpiresIn} is not a valid refresh tokeb date string.`);
        }
  ...
  public async createAccessToken(params: { userId: string; authType: string }): Promise<string> {
    const payload: any = {
      sub: params.userId,
