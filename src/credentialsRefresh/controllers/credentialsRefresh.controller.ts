...
>(params: { mapCredentials: MapCredentials<Credentials> }): Type<any> {
  ...
  class CredentialsRefreshController {
    ...
    public async refreshCredentials(
      @Body() dto: CredentialsRefreshDto,
    ): Promise<Session<Credentials, User>> {
      const epxiredCredentials: Optional<Credentials> =
        await this.credentialsService.getWithRefreshToken(dto.refresh_token);
      if (!epxiredCredentials) {
        throw new UnauthorizedException("Invalid refresh token.");
      }
      const user: Optional<User> = await this.userService.getWithId(
        epxiredCredentials.userId,
      );
      if (!user) {
        throw new UnauthorizedException(
          `User with id ${epxiredCredentials.userId} not found.`,
        );
      }
      ...
      await this.credentialsService.deleteWithAccessToken(
        epxiredCredentials.accessToken,
      );
